...
use anyhow::Error;
use v8::CreateParams;
use util::error::{custom_error, JsError};

static INITIALIZED: bool = false;

pub struct JSRunner<'a> {
...
impl JSRunner {
    pub fn new(platform: Option<v8::SharedRef<v8::Platform>>, heap_min: usize, heap_max: usize) -> Self {
        ...
        params.heap_limits(heap_min, heap_max);
    ...
    pub fn run(&mut self, source: &[u8]) -> Result<v8::Local<v8::Value>, Error>{
        let mut scope = &mut v8::HandleScope::new(self.isolate);
        let context = v8::Context::new(scope);
        let mut handle_scope = &mut v8::HandleScope::with_context(self.isolate, context);
        ...
        let script = match v8::Script::compile(try_catch, source, Option::None) {
            ...
            None => return Result::Err(Error::new(
                JsError::from_v8_exception(scope, try_catch.exception().unwrap())))
        };
    ...
    fn initialize(platform: Option<v8::SharedRef<v8::Platform>>) {
        ...
        match platform {
            None => v8::V8::initialize_platform(v8::V8::new_default_platform(0, false)),
            Some(platform) =>
